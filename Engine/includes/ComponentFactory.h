...
namespace buki
{
	...
	public:
		...
		static Component* Create(Entity* e, const std::string& typeName, const json& j)
		{
			auto& map = GetMap();
		...
		static const std::type_info* GetType(const std::string& typeName)
		{
			auto& rmap = GetReverseMap();
			for (const auto& [typeInfo, registeredName] : rmap)
			{
