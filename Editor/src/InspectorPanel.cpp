...
void buki::InspectorPanel::Render(EditorState& state)
{
	...
	if (ImGui::CollapsingHeader("Components", ImGuiTreeNodeFlags_DefaultOpen))
	{
		for (auto& [type, component] : entity->GetComponents())
		{
			...
			{
				cmpName = buki::ComponentFactory::GetTypeName(typeid(*dynamic_cast<Text*>(component)));
				changed |= DrawTextComponent(component);
			}
			else if (dynamic_cast<Button*>(component))
			{
				cmpName = buki::ComponentFactory::GetTypeName(typeid(*dynamic_cast<Button*>(component)));
				changed |= DrawButtonComponent(component);
			}
			else if (dynamic_cast<Sprite*>(component))
			{
				cmpName = buki::ComponentFactory::GetTypeName(typeid(*dynamic_cast<Sprite*>(component)));
				changed |= DrawSpriteComponent(component);
			}
			else if (dynamic_cast<TileLayer*>(component))
			{
				cmpName = buki::ComponentFactory::GetTypeName(typeid(*dynamic_cast<TileLayer*>(component)));
				changed |= DrawTileLayerComponent(component, state);
			}
			else if (dynamic_cast<RigidBody*>(component))
			{
				cmpName = buki::ComponentFactory::GetTypeName(typeid(*dynamic_cast<RigidBody*>(component)));
				changed |= DrawRigidBodyComponent(component);
			}
			else if (dynamic_cast<Box*>(component))
			{
				cmpName = buki::ComponentFactory::GetTypeName(typeid(*dynamic_cast<Box*>(component)));
				changed |= DrawBoxComponent(component);
			}
			else if (dynamic_cast<Circle*>(component))
			{
				cmpName = buki::ComponentFactory::GetTypeName(typeid(*dynamic_cast<Circle*>(component)));
				changed |= DrawCircleComponent(component);
			}
			else if (dynamic_cast<Polygon*>(component))
			{
				cmpName = buki::ComponentFactory::GetTypeName(typeid(*dynamic_cast<Polygon*>(component)));
				changed |= DrawPolygonComponent(component);
			}
...
bool buki::InspectorPanel::DrawTextComponent(Component* cmp)
{
	...
	if (ImGui::TreeNodeEx("Text", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		if (ImGui::InputInt("Font Size", text->GetFontSizeRef()))
		{
			changed = true;
		}

		if (ImGui::Checkbox("Center X", text->GetCenterXRef()))
		{
			changed = true;
		}

		if (ImGui::Checkbox("Center Y", text->GetCenterYRef()))
		{
			changed = true;
		}
		...
		{
			text->SetColor({ colorValues[0], colorValues[1], colorValues[2], colorValues[3] });
			changed = true;
		}

		float offset[2] = { text->GetPositionOffset().x, text->GetPositionOffset().y };
		if (ImGui::InputFloat2("Offset", offset, "%.3f"))
		{
			text->SetPositionOffset(Vector2(offset[0], offset[1]));
			changed = true;
		}
	ImGui::TreePop();
	}
...
bool buki::InspectorPanel::DrawButtonComponent(Component* cmp)
{
	...
	if (ImGui::TreeNodeEx("Button", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		if (ImGui::InputText("Message", messageBuffer, sizeof(messageBuffer)))
		{
...
bool buki::InspectorPanel::DrawSpriteComponent(Component* cmp)
{
	...
	if (ImGui::TreeNodeEx("Sprite", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		if (ImGui::InputFloat2("Position Offset", posOffsetValues, "%.3f"))
		{
			sprite->SetPositionOffset({ posOffsetValues[0], posOffsetValues[1] });
			changed = true;
		}

		Vector2 sizeOffset = sprite->GetSizeOffset();
		float sizeOffsetValues[2] = { sizeOffset.x, sizeOffset.y };
		if (ImGui::InputFloat2("Size Offset", sizeOffsetValues, "%.3f"))
		{
			sprite->SetSizeOffset({ sizeOffsetValues[0], sizeOffsetValues[1] });
			changed = true;
		}
		...
		if (ImGui::Checkbox("Use Source Rect", &useSourceRect))
		{
		...
		if (ImGui::InputFloat4("Source Rect", srcValues, "%.3f"))
		{
			sprite->SetSourceRectPixels({ srcValues[0], srcValues[1], srcValues[2], srcValues[3] });
			changed = true;
		}
...
bool buki::InspectorPanel::DrawTileLayerComponent(Component* cmp, EditorState& state)
{
	...
	if (ImGui::TreeNodeEx("TileLayer", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		{
			tileLayer->SetAtlasPath(atlasPathBuffer);
		...
		if (ImGui::DragFloat2("Layer Offset", layerOffsetValues, 0.1f, 0.0f, 0.0f, "%.3f"))
		{
			tileLayer->SetLayerOffset({ layerOffsetValues[0], layerOffsetValues[1] });
			changed = true;
		}

		Vector2 defaultTileSize = tileLayer->GetDefaultTileSize();
		float defaultTileSizeValues[2] = { defaultTileSize.x, defaultTileSize.y };
		if (ImGui::DragFloat2("Default Tile Size", defaultTileSizeValues, 0.1f, 0.0f, 0.0f, "%.3f"))
		{
			tileLayer->SetDefaultTileSize({ defaultTileSizeValues[0], defaultTileSizeValues[1] });
			if (state.setTilesDefaults)
			{
				needRebuild = true;
			}
			changed = true;
		...
		if (ImGui::DragInt4("Default Source Rect", defaultSrcValues, 1, 0, 0))
		{
			tileLayer->SetDefaultSourceRectPixels({ (float)defaultSrcValues[0], (float)defaultSrcValues[1], (float)defaultSrcValues[2], (float)defaultSrcValues[3] });
			if (state.setTilesDefaults)
			{
				needRebuild = true;
			}
		...
		if (ImGui::ColorEdit4("Tint", tintValues))
		{
			tileLayer->SetTint({ tintValues[0], tintValues[1], tintValues[2], tintValues[3] });
			changed = true;
		}
		...
		if (ImGui::Checkbox("Flip X", &flipX))
		{
			tileLayer->SetDefaultFlipX(flipX);
			if (state.setTilesDefaults)
			{
				needRebuild = true;
			}
		...
		if (ImGui::Checkbox("Flip Y", &flipY))
		{
			tileLayer->SetDefaultFlipY(flipY);
			if (state.setTilesDefaults)
			{
				needRebuild = true;
			}
		...
		{
			tileLayer->SetDefaultVisible(visible);
			if (state.setTilesDefaults)
			{
				needRebuild = true;
			}
		...
		if (ImGui::TreeNode("Tiles"))
		{
			for (int i = 0; i < static_cast<int>(tiles.size()); ++i)
			{
				...
				if (ImGui::TreeNode(("Tile " + std::to_string(i)).c_str()))
				{
					...
					if (ImGui::DragFloat2("Local Position", localPos, 0.1f, 0.0f, 0.0f, "%.3f"))
					{
						tile.localPosition = { localPos[0], localPos[1] };
						changed = true;
					}

					float size[2] = { tile.size.x, tile.size.y };
					if (ImGui::DragFloat2("Size", size, 0.1f, 0.0f, 0.0f, "%.3f"))
					{
						tile.size = { size[0], size[1] };
						changed = true;
					}

					int src[4] = {
						(int)tile.sourceRectPixels.x,
						(int)tile.sourceRectPixels.y,
						(int)tile.sourceRectPixels.w,
						(int)tile.sourceRectPixels.h
					};
					if (ImGui::DragInt4("Source Rect", src, 1, 0, 0))
					{
						tile.sourceRectPixels = { (float)src[0], (float)src[1], (float)src[2], (float)src[3] };
						changed = true;
					}
					...
					{
						tile.color = { tileColor[0], tileColor[1], tileColor[2], tileColor[3] };
						changed = true;
					}

					if (ImGui::Checkbox("Flip X", &tile.flipX))
					{
						changed = true;
					}

					if (ImGui::Checkbox("Flip Y", &tile.flipY))
					{
						changed = true;
					}

					if (ImGui::Checkbox("Visible", &tile.visible))
					{
						changed = true;
					}
...
bool buki::InspectorPanel::DrawRigidBodyComponent(Component* cmp)
{
	...
	if (ImGui::TreeNodeEx("RigidBody", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		{
			rigidBody->def.type = static_cast<RigidBody::BodyType>(currentType);
			changed = true;
		}

		if (ImGui::Checkbox("Lock Linear X", &rigidBody->def.motionLocks.linearX))
		{
			changed = true;
		}

		if (ImGui::Checkbox("Lock Linear Y", &rigidBody->def.motionLocks.linearY))
		{
			changed = true;
		}

		if (ImGui::Checkbox("Lock Angular Z", &rigidBody->def.motionLocks.angularZ))
		{
			changed = true;
		}
...
bool buki::InspectorPanel::DrawShapeCommonFields(ShapeDef& def)
{
	...
	if (ImGui::Checkbox("Fill Draw", &def.fillDraw))
	{
		changed = true;
	}

	if (ImGui::Checkbox("Shape Draw", &def.shapeDraw))
	{
		changed = true;
	}
	...
	if (ImGui::ColorEdit4("Shape Color", shapeColor))
	{
		def.shapeColor = { shapeColor[0], shapeColor[1], shapeColor[2], shapeColor[3] };
		changed = true;
	}
	...
	{
		def.fillColor = { fillColor[0], fillColor[1], fillColor[2], fillColor[3] };
		changed = true;
	}

	if (ImGui::Checkbox("Is Sensor", &def.isSensor))
	{
		changed = true;
	}

	if (ImGui::InputInt("Filter", &def.filter))
	{
		changed = true;
	}

	if (ImGui::InputFloat("Density", &def.density, 0.0f, 0.0f, "%.3f"))
	{
		def.density = ClampMinFloat(def.density, 0.0f);
		changed = true;
	}

	if (ImGui::InputFloat("Friction", &def.friction, 0.0f, 0.0f, "%.3f"))
	{
		def.friction = ClampFloat(def.friction, 0.0f, 1.0f);
		changed = true;
	}

	if (ImGui::InputFloat("Restitution", &def.restitution, 0.0f, 0.0f, "%.3f"))
	{
		def.restitution = ClampFloat(def.restitution, 0.0f, 1.0f);
		changed = true;
	}

	float positionOffset[2] = { def.positionOffset.x, def.positionOffset.y };
	if (ImGui::InputFloat2("Position Offset", positionOffset, "%.3f"))
	{
		def.positionOffset = { positionOffset[0], positionOffset[1] };
		changed = true;
	}
...
bool buki::InspectorPanel::DrawBoxComponent(Component* cmp)
{
	...
	if (ImGui::TreeNodeEx("Box", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		if (ImGui::InputFloat2("Size", size, "%.3f"))
		{
			box->def.size = ClampMinVector2({ size[0], size[1] }, 0.001f);
			changed = true;
		}
...
bool buki::InspectorPanel::DrawCircleComponent(Component* cmp)
{
	...
	if (ImGui::TreeNodeEx("Circle", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		if (ImGui::InputFloat("Radius", &circle->def.radius, 0.0f, 0.0f, "%.3f"))
		{
			circle->def.radius = ClampMinFloat(circle->def.radius, 0.001f);
			changed = true;
		}
...
bool buki::InspectorPanel::DrawPolygonComponent(Component* cmp)
{
	...
	if (ImGui::TreeNodeEx("Polygon", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		if (ImGui::InputFloat("Radius", &polygon->def.radius, 0.0f, 0.0f, "%.3f"))
		{
			polygon->def.radius = ClampMinFloat(polygon->def.radius, 0.001f);
			changed = true;
		}

		if (ImGui::InputInt("Segments", &polygon->def.segments))
		{
			polygon->def.segments = ClampMinInt(polygon->def.segments, 3);
			changed = true;
		}
...
{
	std::string name = buki::ComponentFactory::GetTypeName(typeid(cmp));
