...
void buki::AGLevel1::CodeLoad()
{
	...
	spawner->AddPrototype("StoneSlim", new GlassSlim());
...
void buki::AGLevel1::CreateLevelMenu()
{
	...
	buki::Entity* SaveButtonEntity = Instantiate("SaveButton");
	Button* SaveButton = SaveButtonEntity->AddComponent<Button>();
	SaveButton->SetMessage("save");
	SaveButton->SetbuttonFont("./fonts/Kenney/Kenney Blocks.ttf");
	SaveButton->SetButtonText("save", 24);
	SaveButton->ShowBackground(true);
	SaveButton->GetText()->SetColor(Color::BLACK);
	SaveButton->SetOutlineColor(Color::BLACK);
	SaveButtonEntity->GetTransform()->SetPosition(Vector2(0.0f, (-windowSize.y / 2) + (buttonsSize.y * 4)));
	SaveButtonEntity->GetTransform()->SetSize(buttonsSize);
	SaveButton->OnClick.AddListener(this);
	SaveButton->Set();
}
